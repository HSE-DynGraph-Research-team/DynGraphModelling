...
from .caw_graph import CAWNeighborFinder
...
class TGNCAWWrapper(ModelWrapper):
    ...
    default_arg_config = {
        ...
        'use_validation':True,
        'force_cpu':False,
        'pos_dim': 100,#'shape of MLP encoding for each CAW sequence member(dimension of the positional embedding)'
        'pos_enc': 'lp',#['spd', 'lp', 'saw'], 'way to encode distances, shortest-path distance or landing probabilities, or self-based anonymous walk (baseline)')
        'caw_layers': 1, #'number of steps in CAW walks(when only 1 value passed in caw_neighbors)')
        'caw_neighbors':['64', '2'],#'a list of neighbor sampling numbers for different hops, when only a single element is input caw_layers will be activated')
        'use_caw_lstm': True, #'Whether to use LSTM on positional encodings(received from CAWs + MLP)')
        'use_caw': True,#'Whether to add CAW features to messages. 0(False) results in vanilla TGN')
        'use_caw_embed':True, 
        'use_caw_message':True
    ...
    def initialize_model(self, full_data, train_data,node_features, edge_features, batch_params, data_setting_mode):
        ...
        self.max_idx = int(max(full_data.unique_nodes))
        ...
        if self.config['use_caw']:
            self.caw_full_ngh_finder = self.make_caw_neighbor_finder([full_data])
        ...
        self.model = TGN(
            ...
            caw_layers=self.config['caw_layers'], 
            caw_neighbors=self.config['caw_neighbors'], 
            use_caw_lstm=self.config['use_caw_lstm'],
            use_caw=self.config['use_caw'],
            use_caw_message=self.config['use_caw_message'],
            use_caw_embed=self.config['use_caw_embed']
        )
    ...
    def train_self_supervised(self, train_data, val_data, train_sampler, val_sampler  ):
        ...
        if self.config['use_caw']:
            self.caw_partial_ngh_finder = self.make_caw_neighbor_finder([train_data, val_data])
        ...
        for epoch in range(self.config['n_epoch']):
            ...
            if self.config['use_caw']:
                self.model.update_caw_ngh_finder(self.caw_partial_ngh_finder)
            ...
            if self.config['use_caw']:
                self.model.update_caw_ngh_finder(self.caw_full_ngh_finder)
    ...
    def make_caw_neighbor_finder(self, data_sources):
        adj_list = [[] for _ in range(self.max_idx + 1)]
        for data_source in data_sources:
            e,t,s,d = data_source.edge_table[:,:4].T
            e = e.astype(int)
            s = s.astype(int)
            d = d.astype(int)
            for eidx, ts, src, dst in zip(e,t,s,d):
                adj_list[src].append((dst, eidx, ts))
                adj_list[dst].append((src, eidx, ts))
        caw_ngh_finder = CAWNeighborFinder(adj_list, bias=0.0, use_cache=False, sample_method='binary')
        return caw_ngh_finder