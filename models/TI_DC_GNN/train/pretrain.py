...
from models.TI_DC_GNN.train.batching import Batcher
...
class PreTrainer:
    ...
    def train(self, train_data, val_data):
        ...
        batcher = Batcher(self.batch_size, train_data.edge_table)
        for epoch in range(self.train_config['n_epoch']):
            self.logger.info('start {} epoch'.format(epoch))
            start_epoch_time = time.time()
            mean_loss = self.run_train_epoch(batcher)
    ...
    def run_train_epoch(self, batcher):
        loss_lst = []
        batch_cnt = batcher.calc_batch_cnt()
        for i in tqdm(batch_cnt):
            batch = batcher.get_batch_by_num(i)
            cur_loss_lst = self.model.pretrain_local_mp_on_batch(batch)
            loss_lst.append(np.mean(cur_loss_lst))
