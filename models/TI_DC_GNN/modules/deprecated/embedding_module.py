...
from models.TI_GNN_old.model.temporal_attention import TemporalAttentionLayer


class EmbeddingModule(nn.Module):
  def __init__(self, node_memory, node_features,):
...
class GraphEmbedding(EmbeddingModule):
  ...
  def compute_embedding(self, memory, source_nodes, timestamps, n_layers, n_neighbors=20, time_diffs=None,
                        use_time_proj=True):
    """Recursive implementation of curr_layers temporal graph attention functions.

    src_idx_l [batch_size]: users / items input ids.
    cut_time_l [batch_size]: scalar representing the instant of the time where we want to extract the user / item representation.
    curr_layers [scalar]: number of temporal convolutional functions to stack.
    ...
    source_nodes_time_embedding = self.time_encoder(torch.zeros_like(
      timestamps_torch).to(self.device))

    source_node_features = self.node_features[source_nodes_torch, timestamps].to(self.device)
    ...
    if n_layers == 0:
      ...
      edge_features = self.edge_features[edge_idxs].to(self.device)
