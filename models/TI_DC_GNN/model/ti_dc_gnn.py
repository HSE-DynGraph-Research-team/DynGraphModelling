...
from models.TI_DC_GNN.layers.mlp import MergeNodesLayer
...
from ..modules.node_memory import get_node_memory
from ..modules.ti_message_passing.ti_local_message_passing import TiLocalMessagePassing
from ..graph.sampler import get_directed_edge_sampler
from ..graph.ti_global_graph import TiGlobalGraphBuilder
from ..graph.ti_local_graph import TiLocalGraphBuilder
from ..utils.utils import DimensionConfig


class TiDcGNN(nn.Module):
    def __init__(self, node_features, edge_features, device,
                 edge_memory_config, sources, destinations, node_memory_config,
                 local_mp_config, global_mp_config, embedding_module_config):
        super().__init__()
        ...
        self.edge_memory = get_edge_memory(edge_memory_config['memory_init_name'],
                                           ...
                                           fusion_fn_config=edge_memory_config['fusion_fn_name'])
        self.edge_memory_dim = self.edge_memory.memory_dim

        # node memory
        if node_memory_config['use_memory']:
            self.node_memory = get_node_memory(n_nodes=self.n_nodes,
                                               memory_dim=node_memory_config['memory_dim'])
        else:
            self.node_memory = None

        dimension_config = DimensionConfig(node_memory_dim=self.node_memory.memory_dim if self.node_memory is not None else None,
                                                node_features_dim=self.node_features_dim,
                                                edge_memory_dim=self.edge_memory.memory_dim,
                                                edge_features_dim=self.edge_features_dim)

        self.use_local_mp = local_mp_config['use_mp']
        self.use_global_mp = global_mp_config['use_mp']

        # local mp
        self.local_mp = None
        ti_local_graph_builder = None
        if self.use_local_mp:
            self.local_mp = TiLocalMessagePassing(dimension_config=dimension_config,
                                                  device=self.device,
                                                  causal_config=local_mp_config['causal_config'],
                                                  conseq_config=local_mp_config['conseq_config'],
                                                  n_causal_steps=local_mp_config['n_causal_steps'],
                                                  fusion_fn_config=local_mp_config['fusion_fn_config'],
                                                  edge_memory_update_layer_config=local_mp_config['edge_memory_update_config'])
            ti_local_graph_builder = self.init_ti_graph_builder(TiLocalGraphBuilder, ti_local_edge_sampler_config)

        # global mp
        self.global_mp = None
        ti_global_graph_builder = None
        if self.use_global_mp:
            self.global_mp = TiLocalMessagePassing(dimension_config=dimension_config,
                                                  device=self.device,
                                                  causal_config=global_mp_config['causal_config'],
                                                  conseq_config=global_mp_config['conseq_config'],
                                                  n_causal_steps=global_mp_config['n_causal_steps'],
                                                  fusion_fn_config=global_mp_config['fusion_fn_config'],
                                                  edge_memory_update_layer_config=global_mp_config['edge_memory_update_config'])
    ...
    def pretrain_local_mp_on_batch(self, source_nodes, destination_nodes, edge_times, edge_idxs):
        self.mailbox.add_batch(source_nodes, destination_nodes, edge_times, edge_idxs)
        causal_adj_list = self.mailbox.get_causal_adj_list(start_interval_time=edge_times[0])
        self.lo