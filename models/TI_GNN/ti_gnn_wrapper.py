...
import os
import numpy as np
from .model.ti_gnn import TiGNN
from models.TI_GNN.utils.utils import EarlyStopMonitor
from models.TI_GNN.layers.mlp import MLP
from models.TI_GNN.utils.graph_utils import get_neighbor_finder
from collections import defaultdict


class TiGNNWrapper(ModelWrapper):
    default_arg_config = {
    ...
    def initialize_model(self, full_data, train_data, node_features, edge_features, batch_params, data_setting_mode):
        self.device = torch.device(f'cuda:{self.config["gpu"]}' if torch.cuda.is_available() and not self.config[
            'force_cpu'] else 'cpu')
        ...
        self.model = TiGNN(
            ...
            device=self.device,
            ti_neighbor_finder=self.train_ngh_finder,
            embedding_neighbor_finder=self.train_ngh_finder,
            # memory_dimension=self.config['memory_dim'],
            # memory_updater_type=self.config['memory_updater'],
            ti_edge_time_dimension=self.config['ti_edge_time_dimension'],
            ti_n_neighbors=self.config['ti_n_neighbors'],
            embedding_module_type=self.config['embedding_module'],
            embedding_n_neighbors=self.config['embedding_n_neighbors'],
            embedding_n_layers=self.config['embedding_n_layers'],
            embedding_n_heads=self.config['embedding_n_heads'],
