...
def train_val(train_val_data, model, mode, bs, epochs, criterion, optimizer, early_stopper, ngh_finders, rand_samplers, logger):
    ...
    for epoch in range(epochs):
        ...
        val_acc, val_ap, val_f1, val_auc, val_pr, val_rec = eval_one_epoch('val for {} nodes'.format(mode), model, val_rand_sampler, val_src_l,
        ...
        logger.info('val pr: {}'.format(val_pr))
        logger.info('val rec: {}'.format(val_rec))
        if epoch == 0:
