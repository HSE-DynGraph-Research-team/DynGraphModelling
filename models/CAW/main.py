...
VAL_SPLIT_TIME = args.val_split_time
TEST_SPLIT_TIME = args.test_split_time
...
val_time, test_time = list(np.quantile(g_df.ts, [VAL_SPLIT_TIME, TEST_SPLIT_TIME]))
if args.mode == 't':
...
test_acc, test_ap, test_f1, test_auc, test_pr, test_rec = eval_one_epoch('test for {} nodes'.format(args.mode), cawn, test_rand_sampler, test_src_l, test_dst_l, test_ts_l, test_label_l, test_e_idx_l)
logger.info('Eval time: {}'.format(time.time() - eval_start_time))
logger.info('Test time: {}'.format(time.time() - test_start_time))
logger.info('Test statistics: {} all nodes -- acc: {}, auc: {}, ap: {}, f1: {}, pr: {}, rec: {}'.format(args.mode, test_acc, test_auc, test_ap, test_f1, test_pr, test_rec))
test_new_new_acc, test_new_new_ap, test_new_new_auc, test_new_old_acc, test_new_old_ap, test_new_old_auc = [-1]*6
if args.mode == 'i':
    test_new_new_acc, test_new_new_ap, test_new_new_f1, test_new_new_auc, test_new_new_pr, test_new_new_rec = eval_one_epoch('test for {} nodes'.format(args.mode), cawn, test_rand_sampler, test_src_new_new_l, test_dst_new_new_l, test_ts_new_new_l, test_label_new_new_l, test_e_idx_new_new_l)
    logger.info('Test statistics: {} new-new nodes -- acc: {}, auc: {}, ap: {}, f1: {}, pr: {}, rec: {}'.format(args.mode, test_new_new_acc, test_new_new_ap, test_new_new_auc, test_new_new_f1, test_new_new_pr, test_new_new_rec))
    test_new_old_acc, test_new_old_ap, test_new_old_f1, test_new_old_auc, test_new_old_pr, test_new_old_rec = eval_one_epoch('test for {} nodes'.format(args.mode), cawn, test_rand_sampler, test_src_new_old_l, test_dst_new_old_l, test_ts_new_old_l, test_label_new_old_l, test_e_idx_new_old_l)
    logger.info('Test statistics: {} new-old nodes -- acc: {}, auc: {}, ap: {}, f1: {}, pr: {}, rec: {}'.format(args.mode, test_new_old_acc, test_new_old_ap, test_new_old_auc, test_new_old_f1, test_new_old_pr, test_new_old_rec))
