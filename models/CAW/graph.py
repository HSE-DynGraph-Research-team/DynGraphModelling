...
def get_neighbor_finder(
    ...
    **kwargs
    ...
    if isinstance(data_, list):
        table = np.vstack([x.edge_table[:,:4] for x in data_])
    else:
        table = data_.edge_table[:,:4]
    max_node_idx = max(table[:,2].max(), table[:,3].max()) if max_node_idx is None else max_node_idx
    ...
    return NeighborFinder(adj_list, **kwargs)
