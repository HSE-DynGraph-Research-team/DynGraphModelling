...
from models.CAW.transformer import TransformerEncoderLayer, TransformerDecoderLayer
from models.CAW.utils import *
from models.CAW.position import *
...
class LSTMPool(torch.nn.Module):
    def __init__(self, feat_dim, edge_dim, time_dim):
        ...
        self.lstm = torch.nn.GRU(input_size=self.att_dim,
...
class CAWN(torch.nn.Module):
    def __init__(self, n_feat, e_feat, agg='tree',
        ...
        # features
        self.n_feat_th = torch.nn.Parameter(torch.from_numpy(n_feat.get().astype(np.float32)), requires_grad=False)
        self.e_feat_th = torch.nn.Parameter(torch.from_numpy(e_feat.get().astype(np.float32)), requires_grad=False)
...
class FeatureEncoder(nn.Module):
    def __init__(self, in_features, hidden_features, dropout_p=0.1):
        ...
        if self.model_dim == 0:  # meaning that this encoder will be use less
            return
        self.lstm_encoder = nn.GRU(input_size=in_features, hidden_size=self.hidden_features_one_direction, batch_first=True, bidirectional=True)
