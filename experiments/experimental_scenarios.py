...
def run_scenario(model, dataset_name, batch_params, split_params, node_mask_frac, edge_mask_frac, **kwargs):
...
def make_all_configs(task_configs=None):
    ...
    for i, data in task_configs:
        with open(os.path.join(configs_dir, f'{i} config.txt'), 'w') as f:
            for line in data:
                f.write(line+'\n')
    

def run_one_batch(i, model_name,batch_param_name,split_name,node_mask_name,edge_mask_name, run_name, total=None):
...
def process_one_run():
    ...
    model_name,batch_param_name,split_name,node_mask_name,edge_mask_name = read_batch_config(filename)
    run_one_batch(i, model_name,batch_param_name,split_name,node_mask_name,edge_mask_name, run_name, total)
